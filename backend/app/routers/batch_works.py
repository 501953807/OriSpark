...
@router.post("/works/batch-edit", response_model=ApiResponse)
def batch_edit_works(data: dict, db: Session = Depends(get_db)):
    """批量编辑作品标签/项目."""
...
@router.post("/works/{work_id}/metadata", response_model=ApiResponse)
def update_custom_metadata(work_id: str, data: dict, db: Session = Depends(get_db)):
    """更新作品自定义元数据."""
    work = db.query(Work).filter(Work.id == work_id).first()
    if not work:
        raise HTTPException(status_code=404, detail="作品不存在")

    work.custom_metadata = data.get("metadata", {})
    db.commit()
    return ApiResponse(message="元数据已更新")


@router.post("/works/batch-delete", response_model=ApiResponse)
...
@router.delete("/works/{work_id}/permanent", response_model=ApiResponse)
...
def import_folder(data: dict, db: Session = Depends(get_db)):
    """文件夹批量导入 — 递归读取、去重、自动项目创建、生成缩略图."""
    folder_path = data.get("folder_path", "")
    create_projects = data.get("create_projects", True)
        ...
        for fname in sorted(files):
            ...
            try:
                ...
                if not completion_date:
                    ...
                    completion_date = dt.fromtimestamp(os.path.getmtime(fpath)).strftime("%Y-%m-%d")
...
@router.post("/works/empty-trash", response_model=ApiResponse)
...
@router.post("/works/{work_id}/restore", response_model=ApiResponse)
