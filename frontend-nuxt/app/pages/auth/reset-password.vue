...
async function handleSubmit() {
  ...
  const success = await auth.resetPassword(token, form.value.password)
  submitting.value = false
  if (success) {
    navigateTo('/auth/login')
  } else {
    errorMsg.value = auth.error || '重置失败'
  }
}
</script>
  ...
  <div class="auth-layout">
    <div class="auth-left">
      ...
      <div class="auth-left-content">
        ...
        <h2 class="auth-left-title">设置新密码</h2>
        <p class="auth-left-desc">请设置一个安全的密码</p>
      </div>
    </div>
