...
class NotaryManagerService:
    ...
    def _record_audit_step(self, record_id: str, step: str, status: str = "success", detail: str = "") -> None:
        """记录存证审计追踪步骤."""
    ...
    def create_notary_record(
        ...
        self._record_audit_step(record.id, "create", "success",
        ...
        try:
            ...
            self._record_audit_step(record.id, "pending", "success",
                                   f"ECDSA L1 signature completed, sig_path={sig_path}")
        except Exception as e:
            self._record_audit_step(record.id, "pending", "failure",
    ...
    def get_notary_record(self, record_id: str) -> NotaryRecordResponse:
        record = self.db.query(NotaryRecord).filter(NotaryRecord.id == record_id).first()
        if not record:
    ...
    def confirm_notary_record(
        ...
        self._record_audit_step(record.id, "confirm", "success",
                               f"Notary confirmed with tx={transaction_hash or 'N/A'}, block={block_height or 'N/A'}")
        self._record_audit_step(record.id, "cert_generate", "success",
    ...
    def get_c2pa_record(self, work_id: str) -> Optional[C2PARecord]:
        return self.db.query(C2PARecord).filter(C2PARecord.work_id == work_id).first()
    ...
    def confirm_notary_record_with_all_ops(
        ...
        from app.models.notary import Certificate
        record = self.db.query(NotaryRecord).filter(NotaryRecord.id == record_id).first()
        if not record:
            raise HTTPException(status_code=404, detail="存证记录不存在")
