  ...
  <div class="photographer-view">
    ...
    <div v-if="activeTab === 'raw'" class="tab-panel">
      <div class="panel-card">
        <h3 class="section-title">RAW 格式管理</h3>
        <div class="panel-header">
          <p class="section-desc">管理 RAW 格式文件（CR2/NEF/ARW/DNG）及其元数据</p>
        </div>
        <div v-if="store.rawFormats.length === 0 && !store.loading" class="empty-state">
          <span class="empty-icon">📐</span>
          <p>暂无 RAW 格式记录</p>
        </div>
        <table v-else-if="store.rawFormats.length" class="data-table">
          <thead>
            <tr>
              <th>ID</th>
              <th>作品 ID</th>
          ...
          <tbody>
            <tr v-for="raw in store.rawFormats" :key="raw.id">
              <td>{{ raw.id }}</td>
              <td>{{ raw.work_id }}</td>
    ...
    <!-- ========== 艺术微喷 tab ========== -->
    <div v-if="activeTab === 'prints'" class="tab-panel">
      <FineArtPrintPanel />
    </div>
...
import RawBatchPanel from '@/components/photographer/RawBatchPanel.vue'
...
import DigitalDownloadPanel from '@/components/photographer/DigitalDownloadPanel.vue'
import FineArtPrintPanel from '@/components/photographer/FineArtPrintPanel.vue'
...
const tabs = [
  ...
  { key: 'gps', label: '📍 GPS地图' },
  { key: 'raw', label: '📐 RAW管理' },
  { key: 'downloads', label: '⬇️ 数字预设' },
  { key: 'prints', label: '🖼️ 艺术微喷' },
  { key: 'stats', label: '📊 统计' },
]
...
// ── RAW batch modal ───────────────────────────────────────────
const rawBatchVisible = ref(false)
