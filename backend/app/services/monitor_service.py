...
from app.services.logo_detector import (
    generate_mock_ecommerce_results,
)
...
class MonitorService:
    ...
    def create_monitor_task(self, data: MonitorTaskCreate):
        """创建监测任务."""
    ...
    def batch_scan(self, data: ScanRequest):
        """批量手动扫描 (带去重)."""
        now = datetime.now(timezone.utc)
        total_results = 0
        total_dups = 0
    ...
    def update_result(self, result_id: str, data: ResultUpdateRequest):
        """更新监测结果状态 (含白名单学习触发)."""
    ...
    def generate_evidence_package(self, result_id: str, data: EvidencePackageCreate):
        """生成维权证据包."""
        result = self.db.query(MonitorResult).filter(MonitorResult.id == result_id).first()
        if not result:
            raise HTTPException(status_code=404, detail="结果不存在")
    ...
    def compute_fingerprints(self, data: FingerprintRequest):
        """计算并存储作品的感知哈希指纹."""
        work = self.db.query(Work).filter(Work.id == data.work_id).first()
        if not work:
            raise HTTPException(status_code=404, detail="作品不存在")
        ...
        for hash_type in data.hash_types:
            ...
            existing_fp = self.db.query(LocalFingerprint).filter(
    ...
    def compare_fingerprints(self, data: FingerprintCompareRequest):
        """比对两个作品的指纹相似度."""
        fp_a = self.db.query(LocalFingerprint).filter(
            LocalFingerprint.work_id == data.work_id_a,
            LocalFingerprint.hash_type == data.hash_type,
        ).first()
        fp_b = self.db.query(LocalFingerprint).filter(
    ...
    def create_brand_watch(self, data: BrandWatchCreate):
        """注册品牌监测."""
        brand = BrandWatch(
    ...
    def list_brand_watches(self, is_active: Optional[bool] = None):
        """获取品牌监测列表."""
    ...
    def update_brand_watch(self, brand_id: str, data: BrandWatchUpdate):
        """更新品牌监测."""
        brand = self.db.query(BrandWatch).filter(BrandWatch.id == brand_id).first()
        if not brand:
            raise HTTPException(status_code=404, detail="品牌监测不存在")
    ...
    def get_brand_scan_results(self, brand_id: str, status: Optional[str] = None):
        """获取品牌扫描结果."""
        query = self.db.query(BrandScanResult).filter(BrandScanResult.brand_id == brand_id)
        if status:
    ...
    def list_domain_watches(self, is_active: Optional[bool] = None):
        """获取域名监测列表."""
    ...
    def get_dmca_template(self, work_id: str):
        ...
        if not work:
            raise HTTPException(status_code=404, detail="作品不存在")
        infringing_result = self.db.query(MonitorResult).filter(
            MonitorResult.task_id.in_(
                self.db.query(MonitorTask.id).filter(MonitorTask.work_id == work_id)
            ),
    ...
    def check_code_similarity(self, data: CodeSimilarityRequest):
        """检测两个代码片段的相似度."""
        try:
    ...
    def list_whitelist_suggestions(self):
        """获取白名单自动建议."""
        suggestions = get_pending_suggestions(self.db, min_occurrence=2)
        return ApiResponse(
    ...
    def handle_whitelist_action(self, data: WhitelistActionRequest):
        """处理白名单建议 (接受/拒绝)."""
        if data.action not in ("accept", "decline"):
            raise HTTPException(status_code=400, detail="action 必须是 accept 或 decline")
        if data.action == "accept":
            suggestion = accept_suggestion(self.db, data.suggestion_id)
            if not suggestion:
            ...
            suggestion = decline_suggestion(self.db, data.suggestion_id)
            if not suggestion:
                raise HTTPException(status_code=404, detail="建议不存在")
            return ApiResponse(
    ...
    def list_task_priorities(self, platform: Optional[str] = None, min_score: float = 0.0):
        """列出所有监测任务的优先级评分."""
        query = self.db.query(MonitorTask)
        if platform:
            query = query.filter(MonitorTask.platform == platform)
    ...
    def scan_audio_fingerprint(self, task_id: str, top_n: int = 20):
    ...
    def list_audio_matches(self, work_id: Optional[str] = None, min_confidence: float = 0.0):
        """获取音频指纹扫描结果列表."""
    ...
    def scan_text_plagiarism(
        ...
        for source_id, matched_id, sim, shared_terms in top_matches:
            ...
            if normalized_sim < 10.0:
                continue
            task = self.db.query(MonitorTask).filter(MonitorTask.work_id == source_id).first()
            if not task:
                continue
