...
class IprService:
    ...
    def create_ip_registration(self,data: CreateIPRegistrationPayload):
        """创建 IP 登记记录."""
    ...
    def get_ip_registration(self,record_id: str):
        """获取 IP 登记记录详情."""
    ...
    def delete_ip_registration(self,record_id: str):
        """删除 IP 登记记录."""
        record = self.db.query(IPRegistration).filter(IPRegistration.id == record_id).first()
        if not record:
            raise BusinessException("记录不存在", status_code=404)
    ...
    def get_ipr_guidelines(self,jurisdiction: Optional[str] = None):
    ...
    def get_ipr_guidelines_by_type(self,
        ip_type: str,
        jurisdiction: str = "cn",
    ...
    def list_nice_classes(self,
        search: Optional[str] = None,
        creative_only: bool = False):
        """获取尼斯分类列表."""
        _seed_nice_classes(self.db)
        query = self.db.query(NiceClassification)
        if creative_only:
    ...
    def get_class_goods(self,class_no: int):
        """获取指定类别的规范商品/服务项目."""
        _seed_nice_classes(self.db)
    ...
    def recommend_classes(self,data: RecommendClassesPayload):
        ...
        nice_map = {
            c.class_no: c
            for c in self.db.query(NiceClassification).filter(
    ...
    def get_recommend_strategies(self,):
        """获取按创作者类型的预定义组合策略."""
        return ApiResponse(data=[
            {
                "key": k,
    ...
    def get_application_templates(self,
        ...
        query = self.db.query(ApplicationTemplate).filter(ApplicationTemplate.is_active == True)
        if ip_type:
    ...
    def get_application_template(self,template_id: str):
        """获取单个申请表模板详情 (P2.4.9)."""
        _seed_application_templates(self.db)
    ...
    def prefill_application(self,data: PrefillApplicationPayload):
        ...
        # 获取最新存证记录
        notary = self.db.query(NotaryRecord).filter(
            NotaryRecord.work_id == work_id,
            NotaryRecord.status == "confirmed").order_by(NotaryRecord.confirmed_at.desc()).first()
    ...
    def validate_application(self,data: ValidateApplicationPayload):
        """校验申请表单完整性和格式."""
        ip_type = data.ip_type
    ...
    def generate_application(self,data: GenerateApplicationPayload):
    ...
    def export_application(self,data: ExportApplicationPayload):
    ...
    def export_application_package(self,record_id: str):
        """P1.4.8: 生成申请材料 ZIP 包.
    ...
    def get_ip_portfolio(self,):
        """IP 资产组合总览 (P1.4.10)."""
        all_records = self.db.query(IPRegistration).all()
    ...
    def get_reminders(self,):
        """到期提醒列表 (P1.4.11)."""
        all_records = self.db.query(IPRegistration).filter(
    ...
    def get_trademark_gazette_info(self,jurisdiction: str):
        """P2.4.11: 商标公告监测指引.
    ...
    def fee_calculator(self,data: FeeCalculatorPayload):
