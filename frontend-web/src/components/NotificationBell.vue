<template>
  <div class="notif-wrapper">
    <button class="btn btn-ghost notif-btn" @click="togglePanel" title="通知" :class="{ has_unread: unreadCount > 0 }">
      <span class="bell-icon" :class="{ shake: unreadCount > 0 }">
    ...
    <div v-if="panelOpen" class="notif-panel" @click.stop>
      ...
      <div class="notif-list" v-if="notifications.length > 0">
        <div v-for="n in notifications" :key="n.id" class="notif-item" :class="{ unread: !n.is_read }" @click="onClickNotif(n)">
...
const typeLabels: Record<string, string> = {
  cert_ready: '存证完成', scan_result: '扫描结果', reminder: '到期提醒',
  renewal: '续展提醒', order_update: '订单状态', quota_warning: '配额预警',
  backup_complete: '备份完成', system_update: '系统更新',
}
...
onMounted(() => {
  fetchNotifications()
  wsClient.connect()
  const off = wsClient.onMessage(handleWsMsg)
